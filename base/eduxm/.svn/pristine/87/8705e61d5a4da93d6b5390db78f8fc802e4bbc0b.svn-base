...
var arrMatch=[
	...
	new Match("C_ROLES","用户角色","selectfunction","../code/js.json","combobox"),
	new Match("C_ORGCODE","用户组织","selectfunction","../code/org.json","combobox"),
	]; 
...
function errorTip(responseXML, statusText, xhr, form) {
 ...
 function initTable() { 
	 ...
	 getJSON(data, "js","../code/js.json");
	 getJSON(data, "org","../code/org.json");
	 /*$("#tableid").datagrid({
 		title:'用户检索',
 		singleSelect:true,
 		fitColumns:true,
	...
	$('#groupInfoTable').datagrid({    
	    ...
	    columns:[[  
	       ...
	       // {field:'orgcode',title:'用户所属组织',width:fixWidth(0.1)}, 
	        ...
	        {field:'roles',title:'用户角色',width:fixWidth(0.2),formatter:function(value,rowData,index){
	        	return getText(value,data.js);
	        }},
	        {field:'orgcode',title:'用户所属组织',width:fixWidth(0.2),formatter:function(value,rowData,index){
	        	return getText(value,data.org);
	        }},
	...
	$("#p_size").val($("#groupInfoTable").datagrid('options').pageSize);//将每页大小记录下来，放在隐藏域中
}
